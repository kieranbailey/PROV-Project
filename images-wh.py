...
if response.status_code == 200:
    ...
    for item in image_items:
        ...
        image_id = item['_id']
        image_filename = os.path.join(save_directory, f"{image_id}_{image_width}x{image_height}.jpg")

        # Modify the URL to replace the original dimensions with desired dimensions
        modified_url = image_url.replace(original_dimensions, f"{image_width},{image_height}")

        print(f"Downloading image from: {modified_url}")  # Print the modified image URL
        ...
        if image_response.status_code == 200:
            ...
            print(f"Failed to download: {modified_url}")
else:
